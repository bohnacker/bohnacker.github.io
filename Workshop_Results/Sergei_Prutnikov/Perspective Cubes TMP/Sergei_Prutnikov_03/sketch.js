...
let gradient_blue;
function preload() {
  gradient_blue = loadImage("gradient_blue.png");
  gradient_red = loadImage("gradient_red.png");
}
...
function draw() {
  ...
  if (keyIsPressed === true) {
    image(gradient_red, -canvasSize / 2, -canvasSize / 2, canvasSize, canvasSize);
  } else {
    image(gradient_blue, -canvasSize / 2, -canvasSize / 2, canvasSize, canvasSize);
  }
  pop();
      ...
      for (let j = -6; j <= 6; j++) {
        ...
        if ((i + j) % 2 == 0) {
          ...
          if (abs(i) + abs(j) == k * 2) {
            ...
            if (keyIsPressed === true) {
              if (mouseIsPressed) {
                let r = lerp(180, 255, k / 7);
                let g = lerp(160, 219, k / 7);
                let b = lerp(60, 88, k / 7);
                fill(r, g, b);
              } else {
                fill(255, 219, 88);
              }
